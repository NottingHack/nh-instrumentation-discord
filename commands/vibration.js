...
module.exports = function () {
	...
	await fetch(`${conf.prometheusApi}/query_range?query=${query}&start=${startDate}&end=${endDate}&step=1000`)
		...
		var tempEmbed = new EmbedBuilder()
		    .setTitle("Vibration")
		    .setDescription(`The average vibration is ${mean} wobbles.`);

		for (const [k, v] of Object.entries(radiation)) {
		    tempEmbed.addFields(
			{ name: k, value: v.toFixed(2) + ' wobbles', inline: true}
		    );
		};
		...
		await fsp.writeFile('vibrate.png', pngBuffer);
		chart.destroy();

		tempEmbed.setImage('attachment://vibrate.png');
		const file = new AttachmentBuilder('vibrate.png');
